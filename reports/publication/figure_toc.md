...
| 13 | `fig:rf-oof-shap` | `reports/evidence/plots/results/rf_dbz_oof_treeshap.png` | `scripts/rf_oof_treeshap.py`; `scripts/generate_rf_oof_shap_figure.py` | Run the OOF analysis in tmux, then `python scripts/generate_rf_oof_shap_figure.py` | 16 current RF/dB-z refits; 200 trees, depth 8, seed 42; SHAP only on each held-out condition | `alternative_reps.npz`; sample-pooled, condition-balanced, Condition-7-excluded profiles, prediction, and fold-summary CSVs | Primary current-RF explanation; reproduces OOF MAE 0.021009 and displays weighting sensitivity explicitly. |
