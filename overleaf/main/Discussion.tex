...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{discussion_complexity_accuracy.png}
...
equivalent wheel diameter \cite{tonshoff1992acoustic}. For this descriptive calculation, \(a_e=a_p\); \(D_e\) and the r/min-to-surface-speed conversion cancel in the ratio because the same wheel and workpiece diameter apply to both cases. Substituting Condition~7 and the component-wise medians gives
...
\textbf{a}, Mean AE dB spectra. \textbf{b}, Mean vibration dB spectra. The
conditions share workpiece speed and depth of cut but differ in wheel speed
and measured roughness. These descriptive spectra do not identify a causal
mechanism.}\label{fig:condition7-condition10}
\end{figure}
...
prediction is a candidate approach, but with only 16 conditions the calibration set would be small, and coverage would depend strongly on whether calibration conditions span the same process frontier as Condition~7 \cite{Angelopoulos2023}.
...
\midrule
Lowest observed point-estimate group & Random forest &
\shortstack[l]{\texttt{AE-dB-z + Vib-dB-z}\\or\\\texttt{AE-log-mel + Vib-log-mel}} \\
...
\cite{Angelopoulos2023, zhang2023improved}, deep ensembles
