...
point estimate but not significant.  A secondary five-seed sensitivity
analysis supports the robustness of this pattern: after averaging training
stochasticity over five random seeds using the same canonical 14-condition
...
MAEs (Supplementary Table~\ref{tab:supp-seed-repeat-all}).  This
sensitivity analysis is reported as robustness evidence rather than as the
primary inferential conclusion.  The observed RF advantage is most
...
vibration alone achieves 0.0207~\(\upmu\)m, nearly matching the fused
result, while AE alone degrades to 0.0296~\(\upmu\)m. AE therefore
