...
\begin{longtable}{@{}p{2.05cm}p{1.9cm}
...
\path{reports/evidence/tables/canonical_logo_assignments.csv}; cache
...
latency-rf-db-z & Fold-complete timing & 14 & Yes & All 16 canonical repeat-0 checkpoints & Pooled 16,000-call timing; complete checkpoint hashes in \shortstack[l]{\texttt{edge\_latency\_benchmark}\\\texttt{\_per\_fold.csv}} & 0.0200 \\
\bottomrule
\end{longtable}
}
...
model--configuration from \path{full_results_logo_only.csv}. The 240
fold-level prediction files used across its 15 unique model--configuration
sources are indexed in \path{canonical_prediction_manifest.csv}.
