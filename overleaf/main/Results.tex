...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{nested_rf_dbz_logmel_folds.png}
...
\subsubsection{Seed-repeat validation of stochastic
architectures}\label{sec:seed-repeat}

To quantify training stochasticity, the main stochastic architectures
were each repeated over five random seeds under the same 16 LOGO folds
(Supplementary Table~\ref{tab:supp-seed-level}). The repeated neural models
...
and ChannelAttentionCNN 0.0423~\(\upmu\)m) are higher than their seed-42
point estimates because seed 42 lies near the lower-error end of the
observed neural-network seed distribution. Supplementary
...
provides a robustness check for training stochasticity: the RF advantage
persists after averaging out neural-network seed variation. The inference
...
\begin{figure}
\centering
\includegraphics[width=\linewidth]{submission_ranking_top20.png}
...
\begin{figure}
\centering
\includegraphics[width=\linewidth]{submission_prediction_scatter_top3.png}
...
\resizebox{\linewidth}{!}{\input{main/statistical_comparison_table.tex}}
\end{table}
...
\begin{figure}
\centering
\includegraphics[width=\linewidth,height=\textheight,keepaspectratio]{results_statistical_comparison_pairs.png}
...
\begin{figure}
\centering
\includegraphics[width=\linewidth,height=\textheight,keepaspectratio]{results_statistical_comparison_forest.png}
...
\begin{figure}
\centering
\includegraphics[width=\linewidth]{methods_signal_representation_comparison.png}
...
\begin{figure}
\centering
\includegraphics[width=\linewidth,height=\textheight,keepaspectratio]{representation_controlled_comparison.png}
...
sensor-response-limited (Supplementary Figure~\ref{fig:supp-shap-rf}).
...
\begin{figure}
\centering
\includegraphics[width=\linewidth,height=\textheight,keepaspectratio]{results_interpretability_bandmass.png}
...
archived no-cut spectra prevent a causal conclusion. TreeSHAP on vibration ranks bin 209 (\(20.9\) kHz)
...
TreeSHAP on AE peaks at \(1.22\) MHz (bin 182), which lies above the
...
importance; this may represent a structural or chatter-related resonance,
but mounting, sensor-response, preprocessing, or condition-specific
artefacts cannot be excluded. Fold-specific TreeSHAP was computed for the
...
(Supplementary Figure~\ref{fig:supp-shap-rf}). The current refits reproduce
...
\begin{figure}
\centering
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{shap_importance_LightGBMModel_ae_spec+vib_spec.png}
...
\begin{figure}
\centering
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{mc_dropout_intervals_ResNetVibCNN_vib_spec.png}
...
\begin{figure}
\centering
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{mc_dropout_reliability.png}
\caption{Reliability diagram for full 16-fold LOGO MC-dropout on
ResNetVibCNN on Vib-dB. Left: empirical coverage per
uncertainty bin versus the nominal 95\% level. Right: mean absolute error
...
\begin{figure}
\centering
\includegraphics[width=\linewidth,height=\textheight,keepaspectratio]{results_uncertainty_calibration.png}
...
versus observed about 0.360~\(\upmu\)m) and its observed $R_a$ lies above the
...
\begin{figure}
\centering
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{rf_conformal_intervals.png}
...
\begin{figure}
\centering
\includegraphics[width=140mm]{results_condition_error_bars.png}
...
\begin{figure}
\centering
\includegraphics[width=\linewidth]{top_models_fold_mae.png}
...
\begin{figure}
\centering
\includegraphics[width=\linewidth]{top_models_per_condition_mae.png}
...
Table~\ref{tab:modality-ablation} reports a 15-condition robustness RF when
only AE, only vibration, or both modalities are used. Vibration alone
achieves 0.0207~\(\upmu\)m, almost identical to the fused configuration
(0.0202~\(\upmu\)m), whereas AE alone degrades to 0.0296~\(\upmu\)m. Under
...
0.0005~\(\upmu\)m AE-fusion difference is below the stated practical
metrology scale and is not evidence of a practically meaningful average
accuracy gain.

\begin{table}[htbp]
\centering
\caption{Sensor-modality ablation for the 15-condition robustness RF under
16-fold LOGO. Within each fold only the test condition is excluded.}\label{tab:modality-ablation}
\begin{tabular}{@{}lc@{}}
\toprule
Input & MAE (\(\upmu\)m) \\
\midrule
...
\input{main/supp_frequency_ablation.tex}
...
2.3 ms, and their median checkpoints are the smallest (0.35 MB). Their MAE,
however, is \(0.0268\)--\(0.0300~\upmu\)m, worse than the best RF by
34--50\%. LightGBM on \texttt{Vib-dB-z}
...
pooled over the same 16 historical canonical repeat-0 fold checkpoints. Each checkpoint
...
\begin{figure}
\centering
\includegraphics[width=\linewidth]{latency_vs_accuracy.png}
...
\begin{figure}
\centering
\includegraphics[width=\linewidth]{results_deployment_tradeoffs.png}
