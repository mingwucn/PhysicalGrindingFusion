...
\resizebox{\linewidth}{!}{\input{main/supp_holm_table.tex}}
\end{table}
...
\texttt{nested\_logo\_dbz\_matched\_folds.csv} and
\texttt{nested\_logo\_logmel\_matched\_folds.csv}.

\input{main/supp_nested_rf_selections.tex}

\input{main/supp_logo_assignments.tex}

\input{main/supp_rf_experiment_provenance.tex}
...
Vib-dB-z). (a)~Measured $R_a$. (b)~RF prediction residual
($\hat y - y$). (c)~AE broadband RMS. (d)~Tri-axial vibration RMS.
...
(AE-dB-z + Vib-dB-z). Residuals are plotted against measured \(R_a\),
predicted \(R_a\), wheel speed, workpiece speed, depth of cut, run order,
AE RMS, vibration RMS, and the distance outside the training-target
support. Condition~7 samples are highlighted in red.}
...
\input{main/supp_mcdropout_full_logo.tex}

\input{main/supp_mcdropout_deterministic.tex}
...
\texttt{reports/evidence/tables/supp\_model\_inventory.csv} in the
...
Table~\ref{tab:supp-cache-provenance} lists the six principal cached
time--frequency representations used in the manuscript together with
their source quantity, transform chain, normalization order, array shape,
and SHA-256 provenance checksum.

\input{main/supp_cache_provenance.tex}
...
\resizebox{\linewidth}{!}{%
\input{main/supp_logo_extrapolation.tex}
}
\end{table}
...
Figure~\ref{fig:supp-shap-rf} shows out-of-fold TreeSHAP frequency profiles
...
The sample-pooled out-of-fold TreeSHAP mean
absolute importance for the reproducible current random forest trained on
fused AE and vibration dB-z descriptors. A separate RF is fitted in each
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{rf_dbz_oof_treeshap.png}
\caption{Out-of-fold TreeSHAP frequency profiles for the reproducible current
RF on AE-dB-z + Vib-dB-z. Curves show sample-pooled attribution,
equal-condition aggregation after fold-wise normalisation, and the latter
excluding Condition~7. Each held-out pass is explained only by the RF trained
without its condition. This analysis reproduces the current
0.0210~\(\upmu\)m OOF MAE but does not attribute the historical canonical
artifact.}\label{fig:supp-shap-rf}
\end{figure}
...
(42--46).  Condition~7 remains the largest outlier for every model.
...
\input{main/supp_seed_repeat_all.tex}
...
\resizebox{\linewidth}{!}{\input{main/supp_seed_repeat_wilcoxon.tex}}
\end{table}
...
\input{main/supp_seed_repeat_rf.tex}
...
\input{main/supp_seed_level_maes.tex}
\end{table}
...
impurity-based feature-importance estimates. Top: vibration. Bottom: AE.}\label{fig:supp-shap-stability}
\end{figure}
...
\input{main/supp_rf_oob_coverage.tex}
\end{table}
...
To support replication, Table~\ref{tab:supp-dl-arch} details the architectural configurations, layer dimensions, activation functions, regularization (dropout), and parameter counts for the nine architecturally documented deep-learning baselines. The complete 27-model, 38-input canonical LOGO inventory is listed below; models not covered by Table~\ref{tab:supp-dl-arch} are retained as benchmark variants and their archived configurations are named explicitly in the inventory.

\input{main/supp_dl_architectures.tex}

\subsection*{Complete model and input inventory}
\input{main/supp_model_inventory.tex}

Table~\ref{tab:supp-model-constructors} reports the exact constructor
arguments and defaults used by the canonical benchmark for every model in
the inventory. Neural models use the common optimisation protocol stated in
Section~\ref{predictive-models}; model-specific input configuration IDs remain
separate because they determine which constructor branches are active.

\input{main/supp_model_constructor_defaults.tex}
