...
In this study, ``physics-aware'' refers to process-motivated signal
construction, modality-specific frequency interpretation, heuristic physical
indicators, and physics-based consistency checks. It does not denote a
physics-constrained learning objective.
...
\begin{figure}[htbp]
\centering
\resizebox{\linewidth}{!}{\input{tikz/preprocessing_pipeline}}
...
Exact transformations are listed in Supplementary
Table~\ref{tab:supp-cache-definitions}; compact checksum prefixes are given in
Supplementary Table~\ref{tab:supp-cache-provenance}, with complete digests
retained in the public machine-readable manifest.
...
\begin{figure}[htbp]
\centering
\resizebox{\linewidth}{!}{\input{tikz/wst_pipeline}}
...
\begin{figure}
\centering
\includegraphics[width=\linewidth,height=\textheight,keepaspectratio]{methods_signal_representations.png}
\caption{Example signal representations for condition 10, sample 1
($R_a = 0.098~\upmu$m). \textbf{a}, AE dB spectrogram. \textbf{b}, AE
log-mel spectrogram. \textbf{c}, vibration (Z-axis) dB spectrogram.
\textbf{d}, vibration log-mel spectrogram. The examples show dominant lower-frequency
...
because its transfer function is unavailable. Higher-frequency AE activity
above 0.5~MHz and vibration activity near 17~kHz are visible. Their mechanical
or acquisition-chain origins cannot be resolved from the archived sensor
documentation.}
...
\item
  \textbf{Historical shallow MLP:} two hidden layers of 128 and 64 units, L2
  penalty $\alpha=10^{-4}$, Adam optimiser with learning rate
  $10^{-3}$, early stopping on 10\% validation data, maximum 500
  iterations. This archived implementation uses scikit-learn's internal
  random 10\% validation fraction and is the MLP included in the historical
  14-comparison family. The separate \textbf{corrected grouped-validation
  MLP} uses the designated whole validation condition, training-only input
  and target scaling, and a three-candidate training-only grid; it is reported
...
the Results. A secondary equal-budget current fixed-setting analysis repeats
the main stochastic architectures over five seeds; it does not reproduce the
historical tuned checkpoints. Details are provided in
Section~\ref{experimental-workflow}.

\begin{figure}[htbp]
\centering
\resizebox{\linewidth}{!}{\input{tikz/resnetvibcnn_architecture}}
...
\begin{figure}
\centering
\resizebox{\linewidth}{!}{\input{tikz/model_taxonomy}}
\caption{Model taxonomy evaluated in this study. Ridge regression is directly
inspectable; tree ensembles are audited with post-hoc attribution; the shallow
MLP is a separate neural baseline; and deeper neural architectures are analysed
with model-appropriate diagnostics. All families predict $R_a$ and use the same
outer LOGO test folds.}
\label{fig:taxonomy}
\end{figure}
...
The historical shallow MLP uses an internal random 10\% validation fraction.
The corrected grouped-validation MLP instead uses the designated complete
validation condition and is reported separately in Supplementary
Tables~\ref{tab:supp-grouped-mlp}--\ref{tab:supp-grouped-mlp-selections}.}
...
XGBoost              & Number of estimators               & 200 \\
                     ...
                     & Subsample ratio                    & 0.8 \\
\addlinespace[4pt]
Historical shallow MLP & Hidden layers                    & (128, 64) \\
                     & Internal validation fraction       & 10\% \\
                     & L2 regularisation $\alpha$         & $10^{-4}$ \\
Corrected grouped MLP & Validation                        & Whole condition \\
                     & Candidate configurations           & 3 \\
                     & Input and target scaling           & Training only \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}
\centering
\resizebox{\linewidth}{!}{\input{tikz/logo_scheme}}
...
\begin{figure}[htbp]
\centering
\resizebox{0.5\linewidth}{!}{\input{tikz/statistical_workflow}}
...
pipeline is shown in Figure~\ref{fig:rf-oof-shap}. It shows
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{methods_xai_comparison.png}
\caption{Explainability methods applied to real grinding data. \textbf{a},
Ridge regression coefficient magnitudes for Vib-dB. \textbf{b}, TreeSHAP
mean $|\text{SHAP}|$ for LightGBM on Vib-dB.
\textbf{c}, Grad-CAM frequency importance for ResNetVibCNN on Vib-dB.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{methods_uncertainty_overview.png}
\caption{Full 16-fold LOGO MC-dropout uncertainty for ResNetVibCNN on
Vib-dB. \textbf{a}, Nominal 95\% MC-dropout epistemic intervals and observed $R_a$
sorted by predicted mean. \textbf{b}, Reliability diagram: mean predicted
standard deviation versus mean observed absolute error per bin.
\textbf{c}, Empirical coverage per uncertainty bin against the nominal 95\%
...
secondary current fixed-setting sensitivity analysis trained the main stochastic architectures
...
Architecture, preprocessing, validation assignment, and training settings are
fixed within this repeated set while the random seed varies. These settings do
not exactly reproduce the historical tuned checkpoints, so the analysis is a
current-protocol stochastic sensitivity rather than variance around the
historical seed-42 benchmark. It is not treated as the primary inferential
comparison.
...
\begin{figure}[htbp]
\centering
\resizebox{\linewidth}{!}{\input{tikz/workflow_schematic}}
...
archived. The 38-input inventory comprises 25 exactly reconstructable variants
and two archival variants with incomplete resolved-configuration provenance,
