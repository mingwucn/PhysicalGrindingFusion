...
| `scripts/rf_oof_treeshap.py`; `scripts/generate_rf_oof_shap_figure.py` | `reports/evidence/plots/results/rf_dbz_oof_treeshap.png/.pdf`; pooled, condition-balanced, Condition-7-excluded OOF profiles, predictions, and fold summary CSVs | Supplementary Figure `fig:supp-shap-rf`; 16 current RF refits explain held-out conditions only; three profiles expose condition weighting sensitivity |
